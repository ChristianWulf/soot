...
public class EnhancedForStmt extends BranchTargetStmt implements Cloneable, VariableScope {
	...
	public void jimplify2(final Body b) {
		...
		if (getExpr().type().isArrayDecl()) {
			jimplify2ArrayLoop(b);
		} else {
			jimplify2Iterable(b);
		}
	}

	private void jimplify2ArrayLoop(final Body b) {
		...
		b.add(b.newIfStmt(
				...
				end_label(), this));
		b.add(b.newAssignStmt(parameter, asRValue(b, getExpr().type().elementType().emitCastTo(b,
	...
	private void jimplify2Iterable(final Body b) {
