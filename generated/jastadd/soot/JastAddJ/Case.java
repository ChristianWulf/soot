...
public abstract class Case extends Stmt implements Cloneable {
	 ...
	 * @apilevel internal
	 */
	private boolean isDAbefore_compute(final Variable v) {
	...
	private soot.jimple.Stmt label_compute() {
		...
		label.addTag(HigherLevelStructureTags.SWITCH_CASE);
		return label;
	}
	...
	/*
	 * Type checking for case labels need not be changed as it already tests if the case labels have expressions which
	 * are assignable to the switch expression
	 *
	 * @attribute syn
	 *
	 ...
	 */
	@SuppressWarnings({ "unchecked", "cast" })
	public Case bind(final Case c) {
		Object _parameters = c;
		if (bind_Case_values == null) {
			bind_Case_values = new java.util.HashMap(4);
		}
