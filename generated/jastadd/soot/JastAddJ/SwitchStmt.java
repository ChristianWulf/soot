...
public class SwitchStmt extends BranchTargetStmt implements Cloneable {
	...
	public void jimplify2(final Body b) {
		soot.jimple.Stmt cond_label = beginCondLabel();
		...
		int low = map.isEmpty() ? 0 : map.firstKey().intValue();
		int high = map.isEmpty() ? 0 : map.lastKey().intValue();
		...
		if (tableSwitchSize < lookupSwitchSize) {
			java.util.List<soot.jimple.Stmt> targets = new ArrayList<soot.jimple.Stmt>();
			for (int i = low; i <= high; i++) {
				ConstCase ca = map.get(Integer.valueOf(i));
				if (ca != null) {
			...
			b.setLine(this);
			b.add(b.newTableSwitchStmt(expr, low, high, targets, defaultStmt, this));
		} else {
	 ...
	 * @author chw
	 */
	private Stmt beginCondLabel() {
		soot.jimple.Stmt label = newLabel();
		label.addTag(HigherLevelStructureTags.SWITCH_COND);
		return label;
	}
	...
	public void transformation() {
		if (getExpr().type().isEnumDecl()) {
			...
			setExpr(hostType().createEnumMethod(type).createBoundAccess(new List()).qualifiesAccess(
					new ArrayAccess(getExpr().qualifiesAccess(new MethodAccess("ordinal", new List())))));
		}
	 ...
	 *
	 */
	public SwitchStmt(final Expr p0, final Block p1) {
		setChild(p0, 0);
		setChild(p1, 1);
	}
	 ...
	 */
	public boolean assignedAfterLastStmt(final Variable v) {
		ASTNode$State state = state();
		try {
	 ...
	 */
	public boolean unassignedAfterLastStmt(final Variable v) {
		ASTNode$State state = state();
		try {
	...
	private boolean canCompleteNormally_compute() {
		return lastStmtCanCompleteNormally() || noStmts() || noStmtsAfterLastLabel() || noDefaultLabel()
				|| reachableBreak();
	}
	...
	private soot.jimple.Stmt end_label_compute() {
		soot.jimple.Stmt label = newLabel();
		label.addTag(HigherLevelStructureTags.SWITCH_END);
		return label;
	}
