...
public abstract class Expr extends ASTNode<ASTNode> implements Cloneable {
	 ...
	 */
	public SimpleSet keepAccessibleTypes(final SimpleSet oldSet) {
	 ...
	 */
	public SimpleSet keepAccessibleFields(final SimpleSet oldSet) {
		SimpleSet newSet = SimpleSet.emptySet;
		for (Iterator iter = oldSet.iterator(); iter.hasNext();) {
	 ...
	 */
	public boolean mayAccess(final FieldDeclaration f) {
		if (f.isPublic()) {
			return true;
	 ...
	 */
	public Dot qualifiesAccess(final Access access) {
	 ...
	 */
	protected SimpleSet chooseConstructor(final Collection constructors, final List argList) {
	 ...
	 */
	protected static SimpleSet mostSpecific(SimpleSet maxSpecific, final ConstructorDecl decl) {
		if (maxSpecific.isEmpty()) {
	 ...
	 */
	protected soot.Value emitBooleanCondition(final Body b) {
		b.setLine(this);
		soot.jimple.Stmt beginLabel = newLabel(HigherLevelStructureTags.IF_COND_X);
		b.addLabel(beginLabel);
		emitEvalBranch(b);

		soot.jimple.Stmt end_label = endBranchLabel(beginLabel);
	...
	private soot.jimple.Stmt endBranchLabel(final soot.jimple.Stmt beginCond) {// added by chw
		// soot.jimple.Stmt label = newLabel();
		soot.jimple.Stmt label = new JEndNopStmt(beginCond);
		label.addTag(HigherLevelStructureTags.IF_END_X);
		return label;
	}
	 ...
	 */
	public void refined_BooleanExpressions_Expr_emitEvalBranch(final Body b) {
		b.setLine(this);
		if (isTrue()) {
			...
			b.add(b.newIfStmt(b.newEqExpr(asImmediate(b, eval(b)), BooleanType.emitConstant(false), this),
					false_label(), this));
			b.add(b.newGotoStmt(true_label(), this));
		}
	}
	 ...
	 */
	public soot.Value eval(final Body b) {
	 ...
	 */
	public soot.Value emitStore(final Body b, final soot.Value lvalue, final soot.Value rvalue,
	 ...
	 */
	protected void addDependencyIfNeeded(final Collection<Type> set, TypeDecl type) {
	 ...
	 */
	public void emitEvalBranch(final Body b) {
		if (type().isReferenceType()) {
			b.setLine(this);
			b.add(b.newIfStmt(b.newEqExpr(asImmediate(b, type().emitUnboxingOperation(b, eval(b), this)),
					BooleanType.emitConstant(false), this), false_label(), this));
			b.add(b.newGotoStmt(true_label(), this));
	...
	public Constant constant() {
		...
		try {
			throw new UnsupportedOperationException(
					"ConstantExpression operation constant" + " not supported for type " + getClass().getName());
		} finally {
		}
	}

	/*
	 * syn lazy boolean FloatingPointLiteral.isZero() { String s = getLITERAL(); for(int i = 0; i < s.length(); i++) {
	 * char c = s.charAt(i); if(c == 'E' || c == 'e') break; if(Character.isDigit(c) && c != '0') { return false; } }
	 * return true; } syn lazy boolean DoubleLiteral.isZero() { String s = getLITERAL(); for(int i = 0; i < s.length();
	 * i++) { char c = s.charAt(i); if(c == 'E' || c == 'e') break; if(Character.isDigit(c) && c != '0') { return false;
	 * } } return true; }* @attribute syn
	 *
	 * @aspect ConstantExpression
	 *
	...
	/*
	 * representableIn(T) is true if and only if the the expression is a compile-time constant of type byte, char, short
	 * or int, and the value of the expression can be represented (by an expression) in the type T where T must be byte,
	 * char or short.
	 *
	 * @attribute syn
	 *
	 ...
	 */
	public boolean representableIn(final TypeDecl t) {
		ASTNode$State state = state();
		try {
	 ...
	 */
	public boolean isDAafterFalse(final Variable v) {
		ASTNode$State state = state();
		try {
	...
	/*
	 * eq Stmt.isDAafter(Variable v) { //System.out.println("### isDAafter reached in " + getClass().getName()); //throw
	 * new NullPointerException(); throw new Error("Can not compute isDAafter for " + getClass().getName() + " at " +
	 * errorPrefix()); }* @attribute syn
	 *
	 * @aspect DA
	 *
	 * @declaredat /Users/eric/Documents/workspaces/clara-soot/JastAddJ/Java1.4Frontend/DefiniteAssignment.jrag:332
	 */
	public boolean isDAafterTrue(final Variable v) {
		ASTNode$State state = state();
		try {
	 ...
	 */
	public boolean isDUafterFalse(final Variable v) {
		ASTNode$State state = state();
		try {
			if (isTrue()) {
				return true;
	 ...
	 */
	public boolean isDUafterTrue(final Variable v) {
		ASTNode$State state = state();
		try {
			if (isFalse()) {
				return true;
	 ...
	 */
	public SimpleSet mostSpecificConstructor(final Collection constructors) {
		ASTNode$State state = state();
		try {
			SimpleSet maxSpecific = SimpleSet.emptySet;
			for (Iterator iter = constructors.iterator(); iter.hasNext();) {
	 ...
	 */
	public boolean applicableAndAccessible(final ConstructorDecl decl) {
		ASTNode$State state = state();
		try {
			return false;
		} finally {
	 ...
	 */
	public boolean hasQualifiedPackage(final String packageName) {
		ASTNode$State state = state();
		try {
			return false;
		} finally {
	 ...
	 */
	public SimpleSet qualifiedLookupType(final String name) {
		ASTNode$State state = state();
		try {
	 ...
	 */
	public SimpleSet qualifiedLookupVariable(final String name) {
		ASTNode$State state = state();
		try {
	...
	private soot.jimple.Stmt false_label_compute() {
		return getParent().definesLabel() ? condition_false_label() : newLabel(HigherLevelStructureTags.IF_ELSE_X);
	}
	...
	private soot.jimple.Stmt true_label_compute() {
		return getParent().definesLabel() ? condition_true_label() : newLabel(HigherLevelStructureTags.IF_THEN_X);
	}
	 ...
	 */
	public boolean isVariable(final Variable var) {
		ASTNode$State state = state();
		try {
			return false;
		} finally {
	...
	public soot.jimple.Stmt condition_false_label() {
		ASTNode$State state = state();
		soot.jimple.Stmt condition_false_label_value = getParent().Define_soot_jimple_Stmt_condition_false_label(this,
				null);
	...
	public soot.jimple.Stmt condition_true_label() {
		ASTNode$State state = state();
		soot.jimple.Stmt condition_true_label_value = getParent().Define_soot_jimple_Stmt_condition_true_label(this,
				null);
