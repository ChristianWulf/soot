...
public class PackManager {
	...
	public PackManager(final Singletons.Global g) {
	...
	private void init() {
		...
		// Dummy Dava Phase
		/*
		 * Nomair A. Naeem 13th Feb 2006 Added so that Dava Options can be added as phase options rather than main soot
		 * options since they only make sense when decompiling The db phase options are added in soot_options.xml
		 */
	...
	private void addPack(final Pack p) {
	...
	public boolean hasPack(final String phaseName) {
		return getPhase(phaseName) != null;
	}

	public Pack getPack(final String phaseName) {
		Pack p = packNameToPack.get(phaseName);
		return p;
	}

	public boolean hasPhase(final String phaseName) {
		return getPhase(phaseName) != null;
	}

	public HasPhaseOptions getPhase(final String phaseName) {
	...
	public Transform getTransform(final String phaseName) {
	...
	private void preProcessDAVA() {
		if (Options.v().output_format() == Options.output_format_dava) {
			...
			if (!isSourceJavac) {
				/*
				 * It turns out that the exception attributes of a method i.e. those exceptions that a method can throw
				 * are only checked by the Java compiler and not the JVM
				 *
				 * Javac does place this information into the attributes but other compilers dont hence if the source is
				 * not javac then we have to do this fancy analysis to find all the potential exceptions that might get
				 * thrown
				 *
	...
	private void writeOutput(final Iterator<SootClass> classes) {
	...
	private void releaseBodies(final Iterator<SootClass> classes) {
		while (classes.hasNext()) {
	...
	private void postProcessDAVA() {
		...
		for (SootClass s : appClasses) {
			...
			/*
			 * Nomair A. Naeem 1st March 2006 Check if we want to apply transformations one reason we might not want to
			 * do this is when gathering old metrics data!!
			 */
			...
			/*
			 * Nomair A. Naeem 29th Jan 2006 Added hook into going through each decompiled method again Need it for all
			 * the implemented AST analyses
			 */
			for (SootMethod m : s.getMethods()) {
				/*
				 * 3rd April 2006 Fixing RuntimeException caused when you retrieve an active body when one is not
				 * present
				 *
				 */
				if (m.hasActiveBody()) {
		 ...
		 *
		 * SHOULD BE INVOKED ONLY ONCE!!! If interprocedural analyses are turned off they are checked within this
		 * method.
		 *
	...
	private void runBodyPacks(final SootClass c) {
		...
		for (SootMethod m : methodsCopy) {
			...
			if (produceJimple) {
				Body body = m.retrieveActiveBody();
				// Change
				// chw: unexpected behavior: all packs on which "transform()" is invoked, cannot be disabled
				ConditionalBranchFolder.v().transform(body);
				UnreachableCodeEliminator.v().transform(body);
				// DeadAssignmentEliminator.v().transform(body);
				PackManager.v().getTransform("jop.dae").apply(body);
		...
		if (produceDava) {
			...
			/*
			 * January 13th, 2006 SuperFirstStmtHandler might have set SootMethodAddedByDava if it needs to create a new
			 * method.
			 */
	...
	public BafBody convertJimpleBodyToBaf(final SootMethod m) {
	...
	private void writeClass(final SootClass c) {
	...
	private void postProcessXML(final Iterator<SootClass> classes) {
		if (!Options.v().xml_attributes()) {
			return;
		}
		if (Options.v().output_format() != Options.output_format_jimple) {
	...
	private void processXMLForClass(final SootClass c, final TagCollector tc) {
	 ...
	 */
	private void processXMLForClass(final SootClass c) {
		final int format = Options.v().output_format();
		String fileName = SourceLocator.v().getFileNameFor(c, format);
		XMLAttributesPrinter xap = new XMLAttributesPrinter(fileName, SourceLocator.v().getOutputDir());
		xap.printAttrs(c);
	...
	private void releaseBodies(final SootClass cl) {
